...
export async function fetchForecast(city: City, startDate: Date): Promise<Forecast[]> {
  ...
  for( let i = 0; i < DAYS_IN_FUTURE; i++) {
    ...
    const forecast = new Forecast(city.id, date, temperature_2m, wind_speed_10m, wind_gusts_10m, weather_code, precipitation_probability, cloud_cover, uv_index, precipitation)
...
async function fetchWithRetry(cityName: string, url: string, headers: Record<string, string>, maxRetries: number = 5, maxDelay: number = 19000, minDelay: number = 2000): Promise<Response> {  
 ...
 */
export async function batchUpsertForecasts(
  supabase: any,

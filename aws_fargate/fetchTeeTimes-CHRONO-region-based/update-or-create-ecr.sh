...
# ECR Update/Create Script for fetch-tee-times-chrono-region-based
# Usage: ./update-or-create-ecr.sh

set -e  # Exit on any error

# Configuration
ECR_REPOSITORY_NAME="fetch-tee-times-chrono-region-based"
...
if aws ecr describe-repositories --repository-names $ECR_REPOSITORY_NAME --region $REGION --no-paginate >/dev/null 2>&1; then
...
docker rmi fetch-tee-times-chrono-test 2>/dev/null || true
docker rmi fetch-tee-times-chrono-region-based 2>/dev/null || true

# Step 4: Build new image for AWS architecture
echo "🔨 Building new image for AWS (linux/amd64)..."
docker build --platform linux/amd64 -t fetch-tee-times-chrono-region-based .

# Step 5: Tag for ECR
echo "🏷️  Tagging image for ECR..."
docker tag fetch-tee-times-chrono-region-based:latest $ECR_URI:$IMAGE_TAG
